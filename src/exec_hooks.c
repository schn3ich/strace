...
int append_to_buffer(char **buffer, int *current_offset, int *buffer_size, const char *data) {
	...
	*current_offset += sprintf(&((*buffer)[*current_offset]), "%s", data);
	return 0;
}

int append_string_to_json(char **buffer, int *current_offset, int *buffer_size, const char *field_name, char *string) {
	int success = append_to_buffer(buffer, current_offset, buffer_size, ",\"");
	if (success < 0) {
		return -1;
	}
	success = append_to_buffer(buffer, current_offset, buffer_size, field_name);
	if (success < 0) {
		return -1;
	}
	success = append_to_buffer(buffer, current_offset, buffer_size, "\":\"");
	if (success < 0) {
		return -1;
	}
	success = append_to_buffer_data_escaped(buffer, current_offset, buffer_size, string);
	if (success < 0) {
		return -1;
	}
	success = append_to_buffer(buffer, current_offset, buffer_size, "\"");
...
int append_string_array_to_json(char **buffer, int *current_offset, int *buffer_size, const char *field_name, char **data, int data_length) {
	...
	success = append_to_buffer_data_escaped(buffer, current_offset, buffer_size, data[0]);
	if (success < 0) {
		return -1;
	}
	...
	for (int i = 1; i < data_length; i++) {
		...
		success = append_to_buffer_data_escaped(buffer, current_offset, buffer_size, data[i]);
		if (success < 0) {
			return -1;
		}
	}
...
char *build_json(struct syscall_info *syscall_info) {
	...
	success = append_to_buffer(&json_buffer, &current_offset, &buffer_size, "}");
